...
export default function Settings() {
    ...
    const handleLogout = async () => {
        if (!confirm('আপনি কি logout করতে চান?')) return;
        try {
            await signOut(auth);
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', padding: 32 }}>
            ...
            <div style={{ maxWidth: 680, display: 'flex', flexDirection: 'column', gap: 28 }}>
                ...
                <div className="card">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div>
                            <div style={{ fontSize: 14, fontWeight: 500 }}>Logout</div>
                            <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 3 }}>
                                আপনার account থেকে সাইন আউট করুন
                            </div>
                        </div>
                        <button onClick={handleLogout} className="btn btn-danger" style={{ fontSize: 13 }}>
                            🚪 Logout
                        </button>
                    </div>
                </div>
