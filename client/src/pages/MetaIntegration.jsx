...
export default function MetaIntegration() {
    const { plan, canAccess, isAgent } = usePlan();   // isAgent যোগ
    const [channels, setChannels] = useState([]);
    const [messages, setMessages] = useState([]);
    const [showForm, setShowForm] = useState(false);
    const [reviewing, setReviewing] = useState(null);
    ...
    return (
        <div style={S.page}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                ...
                {/* ── Add Channel button — agent হলে দেখাবে না ── */}
                {isAgent ? (
                    <span style={{
                        fontSize: 12, color: 'var(--text-3)', padding: '8px 14px',
                        background: 'var(--bg-tertiary)', borderRadius: 8, border: '1px solid var(--border)',
                    }}>
                        🔒 Channel admin দ্বারা পরিচালিত
                    </span>
                ) : (
                    <button className="btn btn-primary" onClick={() => setShowForm(true)}>+ Add Channel</button>
                )}
            ...
            {tab === 'overview' && (
                channels.length === 0
                    ? <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-3)', fontSize: 14 }}>
                        {isAgent ? 'Admin এখনো কোনো channel access দেননি।' : 'Channel নেই। "+ Add Channel" দিয়ে শুরু করুন।'}
                    </div>
                    : <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>{channels.map(ch => <ChannelCard key={ch._id} channel={ch} onUpdate={loadChannels} onDelete={loadChannels} readOnly={isAgent} />)}</div>
            )}
            ...
            {/* Add form — agent এর জন্য খুলবে না */}
            {showForm && !isAgent && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
                    <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 16, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
                        <MetaApiForm onSuccess={() => { setShowForm(false); loadChannels(); }} onCancel={() => setShowForm(false)} />
