...
const submitToOMS = async (order, userId) => {
    try {
        ...
        // ── Retry logic — OMS server ঘুমিয়ে থাকলে (Render free tier) ──
        // 502/503/504/timeout হলে আবার চেষ্টা করো (server জাগতে সময় লাগে)
        let res;
        let lastErr;
        const maxRetries = 4;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                res = await axios.post(config.apiUrl, payload, {
                    headers,
                    timeout: 60000,   // 60 সেকেন্ড — server জাগার সময় দাও
                });
                break;  // সফল — loop থেকে বের হও
            } catch (err) {
                lastErr = err;
                ...
                if (isRetryable && attempt < maxRetries) {
                    const waitMs = attempt * 8000;  // 8s, 16s, 24s — server জাগতে দাও
                    console.log(`⏳ OMS server busy/sleeping (${status || err.code}), retry ${attempt}/${maxRetries} in ${waitMs / 1000}s...`);
                    await new Promise(r => setTimeout(r, waitMs));
                    continue;
                }
                throw err;  // retry করার মতো না, অথবা শেষ attempt
            }
        }
