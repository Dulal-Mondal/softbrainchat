...
exports.updateOrder = async (req, res) => {
    try {
        ...
        const order = await Order.findOne({ orderId: req.params.orderId, userId: req.ownerId });
        if (!order) return res.status(404).json({ success: false, message: 'Order not found' });
        ...
        if (oms_order_id) {
            order.omsOrderId = oms_order_id;
            order.omsSynced = true;
            order.omsSyncedAt = new Date();
        }
        await order.save();

        // OMS status পরিবর্তন করলে customer কে auto message পাঠাও
        if (statusChanged && order.lastNotifiedStatus !== status) {
            const { notifyCustomer } = require('./order.controller');
